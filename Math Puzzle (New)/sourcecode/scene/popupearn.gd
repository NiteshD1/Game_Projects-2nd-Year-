...
func _ready():
	utils.connect("networkerror", self, "_on_network") 
	update_coin()
	pass
...
func _on_coinadd_pressed():
	audio.get_node("SamplePlayer2D").play("hit")
	get_node("coinadd").set_scale(Vector2(0.55,0.55))
...
func _on_coinadd_released():
	get_node("coinadd").set_scale(Vector2(0.5,0.5))
	pass # replace with function body
